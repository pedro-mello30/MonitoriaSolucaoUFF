...
int main(){
	...
	cout << "############ Calculo de horas dormidas ############" << endl;
	cout << "Que horas voce dormiu (Hd Md) e que acordou (Ha Ma)? (Hd Md Ha Ma)" << endl;
	cin >> h;
	cin >> m;
	cin >> h1;
	cin >> m1;
	...
	while(h+m+h1+m1!=0){
		if (0<=h && h<24 && 0<=h1 && h1<24 && 0<=m && m<60 && 0<=m1 && m1<60){
			m += h*60;
			m1 += h1*60;

			aux = m1-m;
			aux = (aux<=0)?aux+1440:aux;
			cout << aux << endl; 
		}
